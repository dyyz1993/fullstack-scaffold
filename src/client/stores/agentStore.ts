...
const rebuildIndex = (rounds: MessageRound[]): Map<string, MessagePosition> => {
  const index = new Map<string, MessagePosition>()
  rounds.forEach((round, roundIdx) => {
    round.agentMessages.forEach((msg, msgIdx) => {
      index.set(msg.id, { roundIdx, msgIdx })
    })
  })
  return index
}
...
export const useAgentStore = create<AgentState>((set, get) => {
  // Internal index for O(1) message lookups - not exposed on state
  let _messageIndex = new Map<string, MessagePosition>()

  const updateRoundsWithIndex = (newRounds: MessageRound[]) => {
    _messageIndex = rebuildIndex(newRounds)
  ...
  return {
    ...
    sendMessage: async (content: string) => {
      ...
      set(state => updateRoundsWithIndex([...state.rounds, newRound]))
        ...
        if (result.success) {
          ...
          set(state => {
            ...
            const uniqueNewRounds = append
              ...
              : newRounds
            const mergedRounds = append
              ? [...uniqueNewRounds, ...state.rounds]
              : [...uniqueNewRounds].reverse()

            _messageIndex = rebuildIndex(mergedRounds)
    ...
    addAgentMessage: messageId => {
      ...
      set(state => {
        ...
        // Update index for the newly added message
        _messageIndex.set(messageId, {
          roundIdx: lastRoundIndex,
          msgIdx: lastRound.agentMessages.length,
        })
    ...
    addSubRound: (messageId: string) => {
      const pos = _messageIndex.get(messageId)
      if (!pos) return
      ...
      if (lastSubRound?.toolCalls && lastSubRound.toolCalls.length > 0) {
        ...
        newRound.agentMessages[msgIdx] = updatedAgentMsg
    ...
    updateCurrentSubRoundThinking: (messageId: string, updater: (prev: string) => string) => {
      ...
      const newRound = { ...round, agentMessages: [...round.agentMessages] }
      newRound.agentMessages[msgIdx] = { ...agentMsg, subRounds: updatedSubRounds }
      newRounds[roundIdx] = newRound
