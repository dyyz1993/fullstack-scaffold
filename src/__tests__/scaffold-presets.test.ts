...
describe("Scaffold Presets", () => {
  describe("minimal preset", () => {
    ...
    const excludedModules = ALL_MODULES.filter(
    ...
    it("should have only todos shared schemas", () => {
      const content = fs.readFileSync(
        path.join(projectDir, "src/shared/modules/index.ts"),
        "utf-8",
      );
    ...
    it("should not have excluded client stores", () => {
      expect(
        fs.existsSync(
          path.join(projectDir, "src/client/stores/chatWSStore.ts"),
        ),
      ).toBe(false);
      expect(
        fs.existsSync(
          path.join(projectDir, "src/client/stores/notificationStore.ts"),
        ),
      ).toBe(false);
      expect(
        fs.existsSync(path.join(projectDir, "src/client/stores/authStore.ts")),
      ).toBe(false);
    });
    ...
    it("should not have excluded client pages", () => {
      expect(
        fs.existsSync(
          path.join(projectDir, "src/client/pages/WebSocketPage.tsx"),
        ),
      ).toBe(false);
      expect(
        fs.existsSync(
          path.join(projectDir, "src/client/pages/NotificationPage.tsx"),
        ),
      ).toBe(false);
    });
  });
  ...
  describe("todo-app preset", () => {
    ...
    it("should have all 3 client stores", () => {
      expect(
        fs.existsSync(path.join(projectDir, "src/client/stores/todoStore.ts")),
      ).toBe(true);
      expect(
        fs.existsSync(
          path.join(projectDir, "src/client/stores/chatWSStore.ts"),
    ...
    it("should have all 3 client pages", () => {
      expect(
        fs.existsSync(path.join(projectDir, "src/client/pages/TodoPage.tsx")),
      ).toBe(true);
      expect(
        fs.existsSync(
          path.join(projectDir, "src/client/pages/WebSocketPage.tsx"),
  ...
  describe("fullstack-admin preset", () => {
    ...
    it("should have all 11 module directories", () => {
    ...
    it("should have admin directory", () => {
      ...
      expect(fs.existsSync(path.join(projectDir, "src/admin/App.tsx"))).toBe(
        true,
      );
    });
    ...
    it("should have all client stores including auth", () => {
      expect(
        fs.existsSync(path.join(projectDir, "src/client/stores/todoStore.ts")),
      ).toBe(true);
      expect(
