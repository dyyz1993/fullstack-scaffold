...
describe('E2E: Scaffold → Install → Verify', () => {
  ...
  test('step 4: unit tests pass', { timeout: 300_000 }, () => {
    ...
    if (!hasPassed || hasFailed) {
      console.log('⚠️ Unit tests had failures (non-blocking for scaffold verification)')
  ...
  test('step 6: dev server starts and responds', { timeout: 120_000 }, async () => {
    const port = 30999
  ...
  test('step 7: patch-package patches apply correctly', () => {
    const wsClientDts = path.join(projectPath, 'node_modules/hono/dist/types/client/ws-client.d.ts')
    expect(fs.existsSync(wsClientDts), `patched file ${wsClientDts} should exist`).toBe(true)
    ...
    const typesDts = path.join(projectPath, 'node_modules/hono/dist/types/types.d.ts')
    ...
    const clientJs = path.join(projectPath, 'node_modules/hono/dist/client/client.js')
  ...
  test('step 8: database configuration is valid', () => {
    expect(fs.existsSync(path.join(projectPath, 'drizzle.config.ts'))).toBe(true)

    const dbDir = path.join(projectPath, 'src/server/db')
    expect(fs.existsSync(dbDir), 'src/server/db directory should exist').toBe(true)

    const dbFiles = fs.readdirSync(dbDir)
    expect(dbFiles.length, 'db directory should contain migration/schema files').toBeGreaterThan(0)
  })
  ...
  test('step 10: all critical source files exist and are well-formed', () => {
    ...
    for (const file of criticalFiles) {
      expect(fs.existsSync(path.join(projectPath, file)), `${file} should exist`).toBe(true)
    }
