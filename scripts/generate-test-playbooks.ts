    ...
    identities: [
      {
        ...
        cases: [
          ...
          },
          {
            ...
            verify:
              'categories 渲染分类管理表格（历史缺陷：生成器未归一化 manifest /admin 前缀致路由双重前缀不匹配→内容区空白，已修）；permissions 渲染权限矩阵（真实路径带 /system 前缀）；未知深链渲染 404 页不再空白（catch-all）',
    ...
    identities: [
      {
        ...
        cases: [
          ...
          },
          {
            ...
            verify:
              '✅ 已补建（第九轮）+ 路由归一化修复后线上实拍：列表/审核队列/看板/分类四页 + 下架/上架/推荐/删除全操作；对不存在 slug 404 不产生副作用',
          },
          {
            title: '缺名称的插件表单被拒（逆向）',
