...
### 分类管理深链（边界实勘·入口已按 preset 裁剪）

**步骤**:

1. 查看管理后台侧栏——内容管理组下仅内容列表，无分类管理入口
2. 直接深链访问 /admin/categories

**验证**: 实勘（路由归一化修复后复验）：forum preset 不含插件模块，分类管理入口已被侧栏可用性裁剪隐藏；深链渲染 catch-all 404 页（不再静默空白）

**截图**: ![分类管理深链（边界实勘·入口已按 preset 裁剪）](../screenshots/matrix/forum/a09-categories.png)
