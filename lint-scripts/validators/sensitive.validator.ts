...
export function formatSensitiveErrors(errors: SensitiveError[]): string {
  ...
  output += '  ├─────────────────────────────────────────────────────────┤\n'
  output += '  │ Secrets/Keys      │ .env.local / .env.production       │\n'
  output += '  │ Database URLs     │ .env.local (DB_CONNECTION_STRING)  │\n'
  output += '  │ API Endpoints     │ .env.local (API_URL)               │\n'
  output += '  │ External Services │ src/server/config.ts               │\n'
  output += '  │ Port Numbers      │ .env.local (PORT)                  │\n'
  output += '  │ Email Addresses   │ Database or config.ts              │\n'
  output += '  └─────────────────────────────────────────────────────────┘\n'

  output += '\n💡 Example Fix:\n'
  output += '  // ❌ Wrong\n'
  output += '  const apiKey = "sk-1234567890abcdef"\n\n'
  output += '  // ✅ Correct\n'
  output += '  const apiKey = process.env.API_KEY\n'
  output += '  // Then add to .env.local: API_KEY=sk-1234567890abcdef\n'
