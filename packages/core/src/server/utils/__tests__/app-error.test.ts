...
describe('AuthorizationError', () => {
  ...
  it('insufficientRole static with both roles', () => {
    ...
    expect(err.details).toEqual([{ message: "Role 'user' does not meet required role 'admin'" }])
  })
...
describe('ConflictError', () => {
  ...
  it('duplicateEntry static', () => {
    ...
    expect(err.details).toEqual([
      { field: 'email', message: "Value 'test@test.com' already exists" },
    ])
  })
})
...
describe('tryCatch', () => {
  ...
  it('should convert error with default mapper', async () => {
    await expect(tryCatch(() => Promise.reject(new Error('fail')))).rejects.toThrow('fail')
  })
  ...
  it('should convert non-Error throws', async () => {
    await expect(tryCatch(() => Promise.reject('string error'))).rejects.toThrow('string error')
  })
})
