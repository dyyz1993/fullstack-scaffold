...
export function getAppConfig(): AppConfig {
  ...
  const VALID_DRIVERS = ['sqlite', 'mysql', 'd1'] as const
  const rawDriver = typeof process !== 'undefined' ? process.env.DB_DRIVER || 'sqlite' : 'd1'
  const dbDriver: DatabaseDriver = VALID_DRIVERS.includes(rawDriver as DatabaseDriver)
    ? (rawDriver as DatabaseDriver)
    : 'sqlite'
