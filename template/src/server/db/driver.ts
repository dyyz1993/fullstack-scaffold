...
function patchAutoIncrementColumns(): void {
  for (const table of Object.values(schema)) {
    if (table && typeof table === 'object' && TableSymbol in table) {
      const columns = (
        table as Record<
          symbol,
          Record<string, { autoIncrement?: boolean; shouldDisableInsert: () => boolean }>
        >
      )[TableSymbol]
      if (!columns) continue
      for (const col of Object.values(columns)) {
        if (col?.autoIncrement && typeof col.shouldDisableInsert === 'function') {
          const original = col.shouldDisableInsert.bind(col)
          col.shouldDisableInsert = () => original() || true
        }
      }
    }
  }
