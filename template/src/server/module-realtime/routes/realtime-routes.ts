  ...
  const app = new OpenAPIHono<{ Bindings: AppBindings }>()
    ...
    .openapi(wsRoute, async (c) => {
      if (isCloudflare && c.env.NOTIFICATION_DO) {
        ...
        return stub.fetch(c.req.raw) as unknown as Promise<TypedResponse<{ protocol: 'AppWSProtocol'; message: string }, 200, 'json'>>;
      }
    ...
    .openapi(streamRoute, async (c) => {
      if (isCloudflare && c.env.NOTIFICATION_DO) {
        const id = c.env.NOTIFICATION_DO.idFromName('global');
        const stub = c.env.NOTIFICATION_DO.get(id);
        return stub.fetch(c.req.raw) as unknown as Promise<TypedResponse<z.infer<typeof NotificationSchema>, 200, 'sse'>>;
      }
      ...
      }) as unknown as TypedResponse<z.infer<typeof NotificationSchema>, 200, 'sse'>;
    });
