...
import { Upload, Plus, X, Rocket, LogIn } from 'lucide-react'
  ...
  return (
    <div className="min-h-screen" data-testid="publish-page">
      ...
      <div className="max-w-2xl mx-auto px-6 -mt-8 pb-16">
        {!isAuthenticated ? (
          // 未登录守卫（同 /installed 的零依赖模式）：不渲染发布表单，
          // 展示引导卡避免游客看到完整发布入口（服务端 401 之前的体验兜底）
          <div
            className="bg-white rounded-2xl shadow-xl shadow-black/5 border border-gray-100 p-10"
            data-testid="publish-auth-guard"
          >
            <EmptyState
              icon={LogIn}
              title="Sign in to publish a plugin"
              description="Publishing is tied to your developer account. Sign in to share your creation with the community."
            />
            <div className="mt-6 flex justify-center">
              <Link
                to="/login"
                data-testid="publish-signin-link"
