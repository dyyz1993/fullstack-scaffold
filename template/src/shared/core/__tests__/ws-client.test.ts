...
describe('WSClientImpl', () => {
  describe('constructor and status', () => {
    ...
    it('should transition to open on ws.onopen', async () => {
      const { WSClientImpl } = await getModule()
      const client = new WSClientImpl<TestProtocol>('ws://localhost')
      const ws = (client as unknown as { ws: { onopen: ((e: Event) => void) | null; readyState: number } }).ws
    ...
    it('should transition to closed on ws.onclose', async () => {
      const { WSClientImpl } = await getModule()
      const client = new WSClientImpl<TestProtocol>('ws://localhost')
      const ws = (client as unknown as { ws: { onopen: ((e: Event) => void) | null; onclose: ((e: CloseEvent) => void) | null; readyState: number } }).ws
    ...
    it('should not double-transition if already closed on onclose', async () => {
      const { WSClientImpl } = await getModule()
      const client = new WSClientImpl<TestProtocol>('ws://localhost')
      const ws = (client as unknown as { ws: { onclose: ((e: CloseEvent) => void) | null; readyState: number } }).ws
  ...
  describe('call()', () => {
    it('should send RPC request and resolve with result', async () => {
      const { WSClientImpl } = await getModule()
      const client = new WSClientImpl<TestProtocol>('ws://localhost')
      const ws = (client as unknown as { ws: { send: (d: string) => void; onopen: ((e: Event) => void) | null; onmessage: ((e: MessageEvent) => void) | null; readyState: number } }).ws
      ...
      ws.onmessage?.({ data: JSON.stringify({ id: sent.id, result: { message: 'hello', timestamp: 123 } }) } as MessageEvent)
    ...
    it('should reject on RPC error response', async () => {
      ...
      const sent = JSON.parse(sendSpy.mock.calls[0][0] as string)
      ws.onmessage?.({ data: JSON.stringify({ id: sent.id, error: 'Something went wrong' }) } as MessageEvent)
    ...
    it('should reject on timeout', async () => {
      const { WSClientImpl } = await getModule()
      const client = new WSClientImpl<TestProtocol>('ws://localhost')
      const ws = (client as unknown as { ws: { send: (d: string) => void; onopen: ((e: Event) => void) | null; readyState: number } }).ws
      vi.spyOn(ws, 'send')
    ...
    it('should clear timeout timer on resolve', async () => {
      ...
      const sent = JSON.parse(sendSpy.mock.calls[0][0] as string)
      ws.onmessage?.({ data: JSON.stringify({ id: sent.id, result: { message: 'hello', timestamp: 1 } }) } as MessageEvent)
      await callPromise
    ...
    it('should buffer messages when socket is not open', async () => {
      ...
      expect(sendSpy).toHaveBeenCalledTimes(1)
      const sent = JSON.parse(sendSpy.mock.calls[0][0] as string)
      ws.onmessage?.({ data: JSON.stringify({ id: sent.id, result: { message: 'buffered', timestamp: 1 } }) } as MessageEvent)
      await callPromise
    })
  })
  ...
  describe('on()', () => {
    it('should register event handler and receive events', async () => {
      ...
      ws.onmessage?.({ data: JSON.stringify({ type: 'broadcast', payload: { text: 'hello' } }) } as MessageEvent)
    ...
    it('should support multiple handlers for same event', async () => {
      ...
      ws.onmessage?.({ data: JSON.stringify({ type: 'broadcast', payload: { text: 'hi' } }) } as MessageEvent)
    ...
    it('should not call handlers for different event types', async () => {
      ...
      ws.onmessage?.({ data: JSON.stringify({ type: 'notification', payload: { title: 'test' } }) } as MessageEvent)
      expect(handler).not.toHaveBeenCalled()
    })
  })
