...
export const sensitiveConfig: SensitiveConfig = {
  ...
  ignorePatterns: [
    ...
    /\.env\.example$/,
    /__tests__/,
    /lint-scripts\/(validate|config|watch-validator|test-validators-unit|test-tracker|test-history|post-commit-track|smart-test|dev-server|TESTING|README|quick-test|DIRECTORY_RULES)/,
    /server\/config\.ts/,
